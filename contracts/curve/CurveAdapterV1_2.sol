...
contract CurveAdapterV1_2 is RewardDistributionV1 {
	 ...
	 * @dev Reverts if the actual imbalance state doesn't match
	 */
	function verifyImbalance(bool state) public view {
		(uint256 deficientIndex, , ) = checkImbalance();
	 ...
	 * @dev Process:
	 *      0. Opportunistically reconciles any pending profit if the cooldown has elapsed (never reverts the deposit)
	...
	function addLiquidity(uint256 amount, uint256 minShares) external returns (uint256) {
		// Opportunistically settle any pending profit before this deposit, without blocking it
		if (block.number >= latestReconcile + 50000) {
			latestReconcile = block.number;
